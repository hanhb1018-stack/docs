...
Use GragAPI's OpenAI-compatible chat completion and tool-calling interface.
...
curl -X POST "$BASE_URL/v1/chat/completions" \
  ...
  -d '{
    "model": "gpt-4o-mini",
    "messages": [
      {"role": "system", "content": "You are GragAPI's product assistant."},
      {"role": "user", "content": "Introduce GragAPI in three sentences."}
    ],
    "temperature": 0.7
  }'
```
...
GragAPI automatically aligns differences across common compatible interfaces, reducing adaptation cost when switching models.

## GragAPI extension fields

The following fields are GragAPI's extensions on top of the standard OpenAI response body, used to expose billing details of the upstream model.
