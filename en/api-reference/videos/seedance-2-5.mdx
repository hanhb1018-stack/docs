...
For overseas calls, simply change `model` to `dreamina-seedance-2-5`; the endpoint still uses GragAPI's `BASE_URL`. A successful creation returns the task ID:
...
Images, videos, and audio together may total at most 50 items. GragAPI-managed assets can be referenced with `asset://<Asset_Id>`; assets must be uploaded and processed before submitting a generation task.
