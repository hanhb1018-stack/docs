...
Complete ByteDance domestic and overseas real-person verification, LivenessFace asset management, and Seedance video calls through GragAPI.
...
The `model` field in the request body is only used by GragAPI to select the domestic or overseas official channel and is not sent to the ByteDance upstream. We recommend using the same model and `ProjectName` throughout the entire verification and asset workflow.
...
When a domestic request hits the ZLHub channel with real-person capability enabled, `H5Link` is a GragAPI-hosted verification page: it hosts the vendor H5, and after the trusted Group is bound successfully it redirects to `CallbackURL` with the same ByteDance parameters. The caller's flow does not need to distinguish channels.
...
curl -X POST "$BASE_URL/volcark/?Action=CreateVisualValidateSession&Version=2024-01-01" \
  ...
  -d '{
    ...
    "CallbackURL": "https://api.gragapi.com/real-person/callback",
    "ProjectName": "default"
  }'
```
...
`GroupId` is bound by GragAPI to the current API user, official channel, region, and Project. Real-person Groups cannot be created via `CreateAssetGroup`, nor used as the asset group for other users or other regions.
...
* Real-person verification and asset Create/Get/List/Update/Delete currently do not deduct the GragAPI model balance.
...
* The availability of the real-person H5, Group, and Asset depends on the corresponding domestic or overseas official channel entitlement. If the API returns permission or quota errors, contact the GragAPI administrator to confirm the activation status.
