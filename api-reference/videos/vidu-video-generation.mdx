...
通过 GragAPI 创建 Vidu 视频生成任务，支持文生视频、图生视频、首尾帧与参考生视频。
...
- GragAPI 接口：`POST $BASE_URL/vidu/ent/v2/{action}`
...
`task_id` / `id` 为 Vidu 上游原始任务 ID，可直接用于 Vidu 官方查询接口。`platform_id` 为 GragAPI 平台统一追踪 ID（`video_<ULID>`），可用于平台内查询、取消等操作。查询和取消接口同时接受两种 ID。
