...
client = OpenAI(base_url="https://api.gragapi.com/v1", api_key="$TOKEN")
...
```bash
curl -X POST "https://api.gragapi.com/v1/images/edits" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
  "model": "gpt-image-2",
...
```bash
curl -X POST "https://api.gragapi.com/v1/images/generations/async" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
  "model": "gpt-image-2",
...
```bash
curl -X GET "https://api.gragapi.com/v1/images/generations/{id}" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST "https://api.gragapi.com/v1/images/edits/async" \
  -H "Authorization: Bearer $TOKEN" \
  -F "model=gpt-image-2" \
